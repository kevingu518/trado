...
class DevController {
   ...
   * 只在開發環境啟用
   */
  generateSeedData = async (req, res) => {
    // 只在開發環境啟用
    ...
    // 建立測試用戶
    console.log(`👤 建立 ${userCount} 個測試用戶...`);
    const users = [];
    for (let i = 1; i <= userCount; i++) {
      const user = await prisma.user.create({
        data: {
          ...
          name: `Test User ${i}`,
          googleId: `google_${Date.now()}_${i}_${generateRandomString(8)}`,
    ...
    // 為每個用戶建立交易
    console.log(`📊 為每個用戶建立 ${tradesPerUser} 筆交易...`);
    let totalTrades = 0;
      ...
      for (let i = 0; i < tradesPerUser; i++) {
        ...
        const trade = await prisma.trade.create({
          data: {
            ...
            reviewNotes: Math.random() > 0.7 ? `測試交易備註 ${i + 1}` : null,
        ...
        for (let j = 0; j < adjustmentCount; j++) {
          ...
          const timestamp = new Date(createdAt.getTime() + j * 24 * 60 * 60 * 1000);
